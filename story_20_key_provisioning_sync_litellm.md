...
The sync must write a `VerificationToken` row with the Event Engine's metadata context embedded in the `metadata` JSON column. This metadata is the bridge that tells the usage callback (Story 21) which `org_id`, `tenant_id`, and `source_mode` to include in the event payload sent to the Ingest API.
...
| 4 | `metadata` JSON column contains: `{"source_mode": "<mode>", "org_id": "<org>", "tenant_id": "<tenant>", "key_id": "<key_id>"}` |
| 5 | For `source_mode=virtual_key`: `tenant_id` is required and set from the key's assigned tenant |
| 6 | For `source_mode=byok`: `tenant_id` is optional (empty string if not specified); `customer_provider_key` holds the encrypted AI provider key |
...
| 10 | The Redis `KeyContext` JSON matches the exact same fields as the `metadata` column in LiteLLM: `{"key_id", "org_id", "tenant_id", "source_mode", "status"}` |
...
| # | Test | Expected |
|---|---|---|
| TC-01 | Create virtual key via control plane | `VerificationToken` row exists in LiteLLM DB with `source_mode=virtual_key`, correct `org_id`/`tenant_id`/`key_id` in metadata; Redis key exists |
| TC-02 | Create BYOK key with provider key | `VerificationToken.metadata` contains `source_mode=byok` and `customer_provider_key` (encrypted); `tenant_id` is empty string |
...
| **Event Engine Postgres** (`api_keys`) | `SELECT` (source of truth) | `id`, `key_hash`, `org_id`, `tenant_id`, `source_mode`, `status` |
...
- **`metadata` JSON must match `KeyContext` exactly.** Fields: `key_id`, `org_id`, `tenant_id`, `source_mode`. The `status` field is implicit — `blocked=true` means revoked, missing row means deleted, present+not blocked means active.
