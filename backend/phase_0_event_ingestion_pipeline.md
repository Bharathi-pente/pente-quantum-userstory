...
|---|---|---|
| **Org API Key** (`direct_ingest`) | Single org + tenant | Submit events for that org/tenant; `org_id` and `tenant_id` overridden from key context |
| **Virtual Key** (`virtual_key`) | Single org + tenant (derived) | LiteLLM auto-posts usage events; `org_id`/`tenant_id` **must** be derived from key, never trusted from payload |
| **BYOK Key** (`byok`) | Single org + tenant (derived) | LiteLLM auto-posts usage events; org's own AI key used upstream |
| **Unauthenticated** | None | Rejected with `401 UNAUTHORIZED` before any processing |

**Key Principle:** For Virtual Key and BYOK modes, `org_id` and `tenant_id` in the request payload are **never trusted**. They are always overridden with the values from the authenticated key context to prevent cross-tenant spoofing.
...
| # | Criterion |
|---|---|
| 1 | `POST /v1/events` accepts a valid JSON `UsageEvent` payload with required fields (`org_id`, `tenant_id`, `user_id`, `event_type`, `model`, `input_tokens`, `output_tokens`). |
| 2 | Auth middleware extracts `KeyContext` from the `X-API-Key` header (Redis lookup) and injects it into the request context. |
| 3 | For Virtual Key and BYOK modes, `org_id` and `tenant_id` are **overridden** from the `KeyContext`; `source_mode` and `key_id` are **added** to the event. |
| 4 | Idempotency check via Redis `SETNX` on `idem:{org_id}:{event_id}` — duplicate `event_id` returns `409 CONFLICT`. |
| 5 | Org existence and user-in-org validated against Redis cache → PostgreSQL fallback. Unknown org/user returns `403 FORBIDDEN`. |
...
| 13 | Batch org lookup and batch user lookup use Redis pipeline + PostgreSQL batch fallback for efficiency. |
...
**Given:** Valid org API key for `org_acme` / `tenant_acme` in Redis
...
**Given:** Virtual key `sk_live_vk_xyz` mapped to `org_beta` / `tenant_beta` in Redis
**When:** `POST /v1/events` with `org_id=org_evil` (spoof attempt) in payload
**Then:** Event enriched with `org_id=org_beta`, `tenant_id=tenant_beta`, `source_mode=virtual_key` — spoof prevented
...
### TC-07: User not in org
**Given:** `user_id=user_rando` not a member of the authenticated org
**When:** `POST /v1/events`
**Then:** `403 FORBIDDEN`, `error=USER_NOT_IN_ORG`
...
|---|---|---|
| **Redis** (`apikey:{key}`) | `GET` | JSON `KeyContext`: `key_id`, `org_id`, `tenant_id`, `source_mode`, `status` |
| **Redis** (`idem:{org_id}:{event_id}`) | `SETNX` | Idempotency key, TTL 24h |
| **Redis** (`org:{org_id}`) | `GET` (cache) | Org existence flag |
| **Redis** (`org:{org_id}:user:{user_id}`) | `GET` (cache) | User-in-org membership flag |
| **PostgreSQL** (`organizations`) | `SELECT` (fallback) | `id`, `slug`, `status` |
| **PostgreSQL** (`tenants`) | `SELECT` (fallback) | `id`, `org_id`, `slug`, `name`, `status` |
| **PostgreSQL** (`users`) | `SELECT` (fallback) | `id`, `email`, `org_id` |
| **PostgreSQL** (`api_keys`) | — (read via Redis only) | `id`, `key_hash`, `org_id`, `tenant_id`, `source_mode`, `status` |
...
| `VALIDATED` | Required fields present, org/user exist, types correct |
| `DEDUPED` | `event_id` not seen before (Redis SETNX succeeded) |
| `ENRICHED` | `source_mode`, `key_id`, `timestamp_ms` added; `org_id`/`tenant_id` overridden from key context if needed |
...
| `FORBIDDEN` | 403 | Org not found; user not a member of the authenticated org |
| `UNKNOWN_ORG` | 403 | `org_id` (after key override) does not exist in cache or Postgres |
| `USER_NOT_IN_ORG` | 403 | `user_id` is not a member of the authenticated org |
...
- **Redis:** Stores API key contexts as JSON under `apikey:{key_name}`. Stores idempotency keys as `idem:{org_id}:{event_id}` with TTL. Also caches org/user lookups.
- **PostgreSQL:** Source of truth for orgs, users, and API keys. Redis caches are warmed from Postgres.
...
- `internal/cache/redis.go` — Redis client init, idempotency `SETNX`, org/user cache reads/writes
...
- `internal/db/postgres.go` — Postgres client init, `GetOrg()`, `GetOrgBatch()`, `GetUserInOrg()`
...
- `migrations/postgres/001_create_organizations.sql` — Full PostgreSQL DDL: orgs + users + api_keys
...
- **Virtual Key / BYOK security:** `org_id` and `tenant_id` from the payload are **always overridden** by the `KeyContext` for non-direct-ingest modes. This is the critical anti-spoofing measure.
- **Cache Warming on Startup:** A background goroutine queries PostgreSQL to warm Redis with all active keys, active organizations, and user memberships.
...
- [ ] `EnrichFromKeyContext()` — overrides `org_id`/`tenant_id` for proxy modes, sets `source_mode` and `key_id`
...
- [ ] Redis key TTL configuration — org cache and user memberships with 1-hour TTL, API keys with no TTL
- [ ] `POST /v1/events` handler — full pipeline: parse → enrich → normalize → validate → dedup → org/user check → publish → 202
- [ ] `POST /v1/events/batch` handler — wrapped + bare array parsing, Bloom filter dedup, batch org/user lookups, partial success counts
...
- [ ] PostgreSQL migration `001_create_organizations.sql` — creates `organizations`, `users`, `tenants`, `api_keys` tables
