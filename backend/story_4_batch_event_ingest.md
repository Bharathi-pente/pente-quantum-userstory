# Story 4 — Batch Event Ingest (`POST /v1/events/batch`)

> **Phase:** 0 — Core Event Ingestion Pipeline
> **Depends on:** Story 1 (domain types), Story 2 (auth middleware), Story 3 (single event patterns — Kafka writer, Redis idempotency, org/user cache)
> **Blocks:** Nothing (this is the last ingest-specific story)
...
| 10 | For each event: call `event.EnrichFromKeyContext(kc)` and `event.ToUsageEvent()` (same as Story 3) |
...
| All keys from Story 3 | Kafka, Redis, Postgres config | Same defaults |
...
- **Handler placement:** Place in `internal/api/handler.go` alongside the single-event handler from Story 3, or in a separate `internal/api/handler_batch.go`.
