...
|---|---|---|
| 1 | When a request is blocked due to `invalid_key`, write an audit log to PostgreSQL `security_audit_logs`. | Set `org_id="unknown"`, capture client IP, set `violation_type="invalid_key"`, and store the key's prefix (first 8 characters). |
...
| 8 | Supports query parameters to filter results: `org_id` and `violation_type` (must be one of `invalid_key`, `budget_exhausted`, `rate_limit`). | Invalid violation filters return `400` with code `INVALID_VIOLATION_FILTER`. |
...
* **Then**: The IP stored in `security_audit_logs` is `203.0.113.195` (first hop), not the downstream proxy address.
