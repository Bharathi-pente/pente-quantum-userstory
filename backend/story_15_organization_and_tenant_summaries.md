# Story 15 — Organization and Tenant Summaries
...
As a **billing administrator or organization owner**, I need overall summaries and breakdowns of event activity and token usage across my organization and its tenants, so that I can monitor volume trends and bill my customers accurately.
...
*   `GET /v1/orgs/{org_id}/tenants/usage`: Paginated token usage breakdown by tenant.
*   `GET /v1/orgs/{org_id}/models/usage`: Usage breakdown by AI model for the organization.
*   `GET /v1/tenants/{tenant_id}/summary`: Overall metrics for a single tenant.
*   `GET /v1/tenants/{tenant_id}/users/usage`: Paginated token usage breakdown by user for a single tenant.
...
|---|---|---|
| 1 | Ensure the `org_id` and `tenant_id` path parameters are validated for length and pattern; return `400 BAD_REQUEST` with code `INVALID_IDENTIFIER` if empty or malformed. | Checks apply to all endpoints. |
...
| 4 | Paginated endpoints (`tenants/usage` and `users/usage`) must support `limit` (default: 100, max: 1000) and `offset` (default: 0). | Out-of-bounds parameters return `400` with code `INVALID_PAGINATION`. |

### Database Query and Execution Constraints

| # | Criterion | Details / Edge Cases |
|---|---|---|
| 5 | All SQL queries executing against ClickHouse must target `events.usage_events_dedup_v` view and run under a context deadline of 5 seconds. | High-throughput optimization. |
...
| 8 | Ensure multi-tenant isolation: Platform-wide admin requests bypass checks, but Organization manager credentials must match the path `org_id`. | If a non-admin attempts to access metrics of an org they do not belong to, return `403 FORBIDDEN` with code `UNAUTHORIZED_ACCESS`. |
...
### TC-03: Get Organization Usage by Tenant with Pagination
* **Given**: ClickHouse contains event metrics for 12 tenants under `org_acme`.
* **When**: Calling `GET /v1/orgs/org_acme/tenants/usage?limit=5&offset=5`
* **Then**: Returns `200 OK` containing exactly 5 tenant breakdown records (representing tenants 6 through 10), and pagination metadata.
...
### TC-05: Tenant Summary - Org Isolation Check
* **Given**: A client is authenticated with credentials for `org_acme`.
* **When**: Calling `GET /v1/tenants/tenant_under_org_different/summary`
