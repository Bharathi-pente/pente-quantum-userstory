...
- `usage_summary` tracks actual usage per customer/end_user/meter per billing period
...
9. All limit creation, update, override, and revocation events are written to `audit_logs` with actor, target, old/new values, and timestamp.
...
| `audit_logs` | `customer` | INSERT | `id, actor_id, action, target_id, metadata, created_at` |
...
**Note**: EXPIRED and REVOKED are terminal states. A new override must be created to reinstate a customer-specific limit.
...
Accessible from **My Account &rarr; Usage Limits**. Read-only table showing the customer's effective limits (plan-level + any active override). Shows: Meter name, Effective Limit, Period, Current Usage (from `usage_summary`), Usage % progress bar.
...
The API gateway middleware calls `GET /api/v1/usage-limits/check?customer_id=X&meter_id=Y` on each proxied request. Responses:
...
- **Prisma models**: `UsageLimit` (enum `LimitType { SOFT HARD WARNING }` (DB: `soft | hard | warning`), enum `LimitPeriod { MONTHLY YEARLY EVER }` (DB: `monthly | yearly | ever`)), `LimitOverride` (enum `OverrideStatus { ACTIVE EXCEEDED CANCELLED }` (DB: `active | exceeded | cancelled`)), `UsageSummary`
...
- **Usage aggregation**: Use `usage_summary.total_usage` as the source of truth for current usage. The `period_start`/`period_end` must match the limit's `period` (billing period alignment)
- **Audit logging**: Every CUD operation on `usage_limits` and `limit_overrides` must emit an `audit_logs` row with `action` (e.g., `USAGE_LIMIT_CREATED`, `LIMIT_OVERRIDE_CREATED`, `LIMIT_OVERRIDE_REVOKED`), actor ID, target ID, and JSON `metadata` containing old/new values
...
- **Column name alignment**: All queries must use exact ERD column names — no aliasing or abbreviation in generated SQL/Prisma
